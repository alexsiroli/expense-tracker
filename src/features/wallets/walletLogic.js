...
export function addWallet(wallets, newWallet) {
  ...
  return [...wallets, { ...newWallet, id: Date.now(), balance: newWallet.balance ?? 0, initialBalance: newWallet.initialBalance ?? 0 }];
}
...
export function calculateWalletBalance(wallet, expenses, incomes) {
  if (!wallet) return 0;
  const totalIn = incomes.filter(i => i.walletId === wallet.id).reduce((sum, i) => sum + parseFloat(i.amount), 0);
  const totalOut = expenses.filter(e => e.walletId === wallet.id).reduce((sum, e) => sum + parseFloat(e.amount), 0);
